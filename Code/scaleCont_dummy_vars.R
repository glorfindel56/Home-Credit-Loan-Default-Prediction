...
data <- subset(app_data,select=-c(CODE_GENDER_NUM,AGE_IN_YEARS_NUM,HAS_CHILDREN_NUM,MARRIED,MARRIED_NUM,INCOME_BRACKET_NUM,EDUCATION_LEVEL_NUM))

#scale the continous data!!!!
columns_to_scale <- c('AMT_INCOME_TOTAL' ,'AMT_CREDIT' ,'AMT_ANNUITY' ,
                      'AMT_GOODS_PRICE' ,'DAYS_BIRTH' ,'DAYS_EMPLOYED' ,
                      'EXT_SOURCE_1','EXT_SOURCE_2' ,'OBS_30_CNT_SOCIAL_CIRCLE',
                      'DEF_30_CNT_SOCIAL_CIRCLE' ,'OBS_60_CNT_SOCIAL_CIRCLE' ,
                      'DEF_60_CNT_SOCIAL_CIRCLE' ,'DAYS_LAST_PHONE_CHANGE' ,
                      'AMT_REQ_CREDIT_BUREAU_YEAR' ,'HAS_CHILDREN' ,
                      'AGE_IN_YEARS' ,'EMPLOYED_IN_YEARS')
...
data$OCCUPATION_TYPE <- factor(data$OCCUPATION_TYPE, 
                               levels = c(levels(data$OCCUPATION_TYPE), "Unknown"))
...
cat_vars <-c("CODE_GENDER","NAME_CONTRACT_TYPE","FLAG_OWN_CAR",
             "NAME_EDUCATION_TYPE","NAME_FAMILY_STATUS","OCCUPATION_TYPE",
             "EDUCATION_LEVEL","AGE_BUCKET","INCOME_BRACKET")
